...
export class TodoService {
  ...
  getTodo(user: iUser[]): void {
    this.arrayTodo = this.arrayTodo.map((todo) => {
      const UserFound = user.find((user) => user.id === todo.userId);

      if (!UserFound) return todo;
  ...
  filterCompletedTodo(arr: iTodo[]) {
    return this.arrayTodo.filter((dati) => dati.completed === true);
  }
}
